...
def evaluate(params):
    ...
    # Check if either battle bot is a HumanBattleAgent
    has_human_player = any(isinstance(bot, agents.HumanBattleAgent) for bot in battle_bots)
    ...
    while game.winner is None:
        ...
        # Print action if there's a human player
        # if has_human_player and game.phase != engine.Phase.DECK_BUILDING:
        #     print_action(action, game)
...
def run():
    ...
    if args.processes > 1:
        sys.stderr.write("Multiple processes are not implemented in this version. Using single process.\n")
        args.processes = 1
...
# def print_action(action, state):
#     """Print detailed information about an action."""
#     if action.type.name == "PASS":
#         print("Action: PASS")
#         return
        
#     if action.type.name in ["PICK", "CHOOSE"]:
#         card = state.cards[action.origin]
#         print(f"Action: {action.type.name} card {action.origin} ({card})")
#         return
        
#     # For SUMMON, USE, and ATTACK actions
#     origin_desc = str(action.origin)
#     if hasattr(action.origin, 'id'):  # If origin is a card object
#         origin_desc = f"{action.origin.id}"
        
#     target_desc = ""
#     if action.target is not None:
#         if hasattr(action.target, 'id'):  # If target is a card object
#             target_desc = f" targeting card {action.target.id}"
#         elif isinstance(action.target, int):
#             if action.target == -1:
#                 target_desc = " targeting opponent"
#             else:
#                 target_desc = f" targeting card {action.target}"
                
#     print(f"Action: {action.type.name} card {origin_desc}{target_desc}")
